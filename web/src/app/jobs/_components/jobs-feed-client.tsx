...
export function OpenJobsList({ initialJobs }: { initialJobs: JobWithAuthor[] }) {
  ...
  function loadMore() {
    ...
    startTransition(async () => {
      const more = await loadMoreOpenJobs(cursor);
  ...
  return (
    <div>
      ...
      {hasMore && <LoadMoreButton onClick={loadMore} isPending={isPending} />}
    </div>
  );
}
...
export function MyJobsList({
  ...
  function loadMore() {
    ...
    if (!cursor || isPending) return;
    startTransition(async () => {
      const more = await loadMoreMyJobs(userId, cursor);
      if (more.length > 0) setJobs((prev) => [...prev, ...more]);
      setHasMore(more.length === PAGE_SIZE);
    });
  }
...
export function MyBidsList({
  ...
  function loadMore() {
    ...
    if (!cursor || isPending) return;
    startTransition(async () => {
      const more = await loadMoreMyBids(userId, cursor);
